...
```bash
echo 'export PATH=$PATH:$HOME/bin' >> ~/.bashrc  ## path değişse bile executable path değişmesin diye kalıcı olarak .bashrc ye yazdık
```
...
```bash
curl --silent --location "https://github.com/weaveworks/eksctl/releases/latest/download/eksctl_$(uname -s)_amd64.tar.gz" | tar xz -C /tmp  ## eksctl yükleme (.tar.gz zipli anlamına geliyor) 
```
