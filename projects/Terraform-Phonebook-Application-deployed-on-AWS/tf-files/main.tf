...
resource "aws_launch_template" "asg-lt" {
  ...
  user_data = filebase64("user-data.sh") # Userdata'yı klasörden çeker. aynı path de olması lazım
...
resource "aws_db_instance" "db-server" {
  ...
  username = "admin"
  password = "Oliver_1" # değiştirme projede böyle
...
resource "github_repository_file" "dbendpoint" {
  ...
  repository = "phonebooktf" # repo adı yazılacak önemli
