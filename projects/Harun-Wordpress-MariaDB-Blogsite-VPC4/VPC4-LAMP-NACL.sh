...
# 1.Create Sec.Groups:
   ...
   NAT-SG: In bound : "HTTP 80, HTTPS 443, SSH 22  > anywhere (0:/00000)" # No need for NAT-SG if you use NAT Gateway.

# 2.Create EC2 that is installed LAMP with user data seen below for "Wordpress app in Public Subnet 1b"

   VPC: "harun-vpc-a"
   Subnet: "harun-az1b-public-subnet"
...
# 3.Create MariaDB ec2 instance in Private Subnet 1b
   ...
   VPC: "harun-vpc-a"
   Subnet: "harun-az1b-private-subnet"
...
# (You can also create a NAT Gateway for outbound connectivity)
    ...
    AMI: "NAT"
    VPC: "harun-vpc-a"
    Subnet: "harun-az1a-public-subnet"
    Sec Group: "NAT-SG"

# 13. Edit "harun-private-rt" route table:
...
# 19. Create new database named "harundb".
CREATE DATABASE harundb;

# 20. Create a user named "admin".
CREATE USER admin IDENTIFIED BY '123456789';

# 21. Grant permissions to the user "admin" for database "harundb".
GRANT ALL ON harundb.* TO admin IDENTIFIED BY '123456789' WITH GRANT OPTION;  
...
# 1. Create a private ec2 instance in Private Subnet 1a
    ...
    VPC: "harun-vpc-a"
    Subnet: "harun-az1a-private-subnet"
...
# 4. Select Subnet associations sub-menu ---> Edit subnet association ---> select "harun-az1a-private-subnet" ---> edit
...
# 6. Go to the NACL named "harun-private1a-nacl"
...
# 10. Go to the NACL named "harun-private1a-nacl"
